...
from .mix_fix1 import OptimizedHyperMoENeck
...
class slowfastnet(nn.Module):
    def __init__(self, num_classes, fp16=False, num_frame=5):
        ...
        self.neck = OptimizedHyperMoENeck(
    ...
    def forward(self, inputs, domain_ids=None):
        feat = []  
        for i in range(self.num_frame):
            x = self.backbone(inputs[:,:,i,:,:])
            y = self.conv1(torch.cat([self.mulit1(x[2]), x[1]],dim=1))
            fea = self.conv2(torch.cat([self.mulit2(y), x[0]], dim=1))
            feat.append(fea)

        if self.neck:
            feat, losses = self.neck(feat, domain_ids, return_losses=True)

...
class seqDataset(Dataset):
    ...
    def __getitem__(self, index):
        images, box, domain_id = self.get_data(index)
        images = np.transpose(preprocess(images),(3, 0, 1, 2))
        if len(box) != 0:
            box[:, 2:4] = box[:, 2:4] - box[:, 0:2]
            box[:, 0:2] = box[:, 0:2] + ( box[:, 2:4] / 2 )
        return images, box, domain_id   
    ...
    def get_data(self, index):
        ...
        if self.aug is True:
            pass
        return image_data, label_data, domain_id

def dataset_collate(batch):
    images = []
    bboxes = []
    domain_ids = []
    for img, box, domain_id in batch:
        images.append(img)
        bboxes.append(box)
        domain_ids.append(domain_id)
    images = torch.from_numpy(np.array(images)).type(torch.FloatTensor)
    bboxes = [torch.from_numpy(ann).type(torch.FloatTensor) for ann in bboxes]
    domain_ids = torch.tensor(domain_ids).type(torch.LongTensor)
    return images, bboxes, domain_ids

if __name__ == "__main__":
    train_dataset = seqDataset("/home/dww/OD/Work6/data/coco_train_multi.txt", 256, 5, 'train')
